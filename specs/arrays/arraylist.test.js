...
class ArrayList {
  ...
  push(value) {
    ...
    this.length++;
    // return the length after pushing
    return this.length;
  }

  pop() {
    // pop item from the end of arr
    if (this.length === 0) return undefined;
    const value = this.data[this.length - 1];
    // If I only use this.length-- directly, the last element will still be present in the data object, but the length will only be reduced
  ...
  delete(index) {
    // delete an item and handle the array after deletion
    if (this.length === 0) return undefined;
